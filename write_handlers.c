...
/**
 * handle_write_char - Function to Prints a character
 * @c: char types.
	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & FLAG_1 && padd == ' ')
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[ind], length) + write(1, &buffer[1], i - 1));
		}
		else if (!(flags & FLAG_1) && padd == ' ')
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[1], i - 1) + write(1, &buffer[ind], length));
		}
		else if (!(flags & FLAG_1) && padd == '0')
		{
			if (extra_c)
...
/**
 * write_number - Funvtion to print a string
	...
	int flags, int width, int precision, int size)
{
	...
	if ((flags & FLAG_4) && !(flags & FLAG_1))
		padd = '0';
	if (is_negative)
		extra_ch = '-';
	else if (flags & FLAG_2)
		extra_ch = '+';
	else if (flags & FLAG_16)
		extra_ch = ' ';
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		for (i = 3; i < width - length + 3; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & FLAG_1 && padd == ' ')/* Asign extra char to left of buffer */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
		...
		else if (!(flags & FLAG_1) && padd == ' ')/* extra char to left of buffer */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
		...
		else if (!(flags & FLAG_1) && padd == '0')/* extra char to left of padd */
		{
			if (extra_c)
				buffer[--padd_start] = extra_c;
			buffer[1] = '0';
